...
class Recording:
    ...
    def accept(self, payload):
        if (not isinstance(payload, dict) or set(payload) != {"action", "locator"}
                or payload.get("action") not in ("click", "fill", "select", "wait", "read_result_single")
        ...
        if payload["action"] == "read_result_single":
            tag = payload["locator"].split(" > ")[-1].split(":", 1)[0]
...
def _workbook(rows, dropped=0, result_blocks=1):
    wb = Workbook()
    steps = wb.active
    steps.title = "steps"
