...
def debug_fetch(url: str):
    ...
    log(f"=== DEBUG FETCH START: {url} ===")

    chrome_path = os.environ.get("CHROME_EXECUTABLE_PATH", "") or None
    log(f"CHROME_EXECUTABLE_PATH env: {chrome_path or '(not set, auto-detect)'}")
    ...
    try:
        with sync_playwright() as pw:
            ...
            log(f"Launching Chromium with kwargs: {launch_kwargs}")
            browser = pw.chromium.launch(**launch_kwargs)
            log("Browser launched OK")

            page = browser.new_page(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0")
            log("Page created")
            ...
            def on_request(req):
                all_requests.append({"method": req.method, "url": req.url[:120], "type": req.resource_type})

            def on_response(resp):
                all_responses.append({"url": resp.url[:120], "status": resp.status, "type": resp.request.resource_type})
            ...
            log("Listeners registered (request, response, console, pageerror)")

            log("Navigating...")
            t0 = _time.time()
            response = page.goto(url, timeout=60000, wait_until="domcontentloaded")
            log(f"Navigation done in {_time.time()-t0:.1f}s, status={response.status if response else 'None'}")
            log(f"Page URL after load: {page.url}")

            log("Sleeping 8s for AJAX...")
            _time.sleep(8)
            log("Sleep done")
            ...
            for r in all_requests:
                log(f"  {r['method']} {r['type']:8s} {r['url']}")

            log(f"\n--- ALL RESPONSES ({len(all_responses)}) ---")
            for r in all_responses:
                log(f"  [{r['status']}] {r['type']:8s} {r['url']}")

            xhr_responses = [r for r in all_responses if r["type"] in ("xhr", "fetch")]
            log(f"\n--- XHR/FETCH responses: {len(xhr_responses)} ---")
            ...
            if len(console_msgs) > 20:
                log(f"  ... and {len(console_msgs)-20} more")
                    ...
                    result = page.evaluate(
                        """async (url) => {
                            try {
                                ...
                                return JSON.stringify({status: resp.status, length: text.length, preview: text.substring(0, 200)});
                            } catch(e) { return JSON.stringify({error: e.message}); }
                        }""",
                        api_url,
                    )
                    log(f"  {api_url} → {result}")
                except Exception as e:
                    log(f"  {api_url} → ERROR: {e}")
